...
export class UsersService {
  ...
  async register(createUserDto: CreateUserDto): Promise<Users> {
    // register function that creates token, saves user and sends confirmation mail
  ...
  async verifyEmail(
    ...
    // async verifyEmail service to work when email link is entered
  ...
  async checkVerification(username: string): Promise<boolean> {
    // async checkVerification to see if user isVerified: boolean value true
    const user = await this.userRepository.findOne({ where: { username } });
