...
describe('Users Controller END TO END TEST', () => {
  ...
  it('/user/register (POST)', async () => {
    ...
    if (response.body) {
      // this has to be checked to make sure POST request is awaited and not undefined
  ...
  it('/user/check-verification/:username (GET)', async () => {
    const response = await request(app.getHttpServer()).get(
      '/user/check-verification/userthatexists',
    );

    if (response.body) {
      // this has to be checked to make sure GET request is awaited and not undefined
      const res = JSON.stringify(response.body);
      expect(res).toContain('success'); // status: success situation
    }
  });
  ...
  it('/user/check-verification/:username (GET)', async () => {
    await request(app.getHttpServer())
      .get('/user/check-verification/userthatdoesnotexist')
      .expect(404); // username not found 404 situation
  });
